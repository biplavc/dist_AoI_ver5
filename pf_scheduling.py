...
def pf_scheduling(I, drones_coverage, folder_name, deployment, packet_upload_loss_thresh, packet_download_loss_thresh, periodicity, adj_matrix, tx_rx_pairs, tx_users, RB_needed_UL, RB_needed_DL, BS_location, user_locations, MAX_STEPS):  ## maximal age difference
    ...
    for ep in range(random_episodes): # how many times the random policy will be run, similar to episode
        if ep%log_interval_random ==0 and ep!=0:
            ...
            print(f"PF ep = {ep} took {time_gap} seconds from start, rate = {round((1/rate),2)} eps/sec, finish_time = {finish_time} ", flush = True)
        ep_reward = 0
        ep_peak_reward = 0
    ...
    pickle.dump(mad_UL_schedule, open(folder_name + "/" + deployment + "/" + "pf_UL_schedule_" + str(I) + "U.pickle", "wb"))    
    pickle.dump(mad_DL_schedule, open(folder_name + "/" + deployment + "/" + "pf_DL_schedule_" + str(I) + "U.pickle", "wb"))  
