...
def find_rr_action(eval_env, slot): ## maximal age difference
    ...
    if eval_env.prev_action_UL == None and eval_env.prev_action_DL == None:
        download_user_pairs = eval_env.tx_rx_pairs
        upload_users   = eval_env.tx_users
        eval_env.prev_action_DL = download_user_pairs
        eval_env.prev_action_UL = upload_users
        
    else:
        ul_new = eval_env.prev_action_UL
        dl_new = eval_env.prev_action_DL
        ul = ul_new.pop()
        dl = dl_new.pop()
        ul_new.insert(0, ul)
        dl_new.insert(0, dl)
        download_user_pairs = dl_new
        upload_users   = ul_new
        eval_env.prev_action_DL = download_user_pairs
        eval_env.prev_action_UL = upload_users
 ...
 ############################################
    ...
    assert actual_rr_action!=None
    return actual_rr_action    


def rr_scheduling(I, drones_coverage, folder_name, deployment, packet_update_loss, packet_sample_loss, periodicity, adj_matrix, tx_rx_pairs, tx_users, RB_needed_UL, RB_needed_DL):  ## maximal age difference
    
    print(f"\nRR started for {I} users , coverage = {drones_coverage} with update_loss = {packet_update_loss}, sample_loss = {packet_sample_loss}, periodicity = {periodicity}, tx_rx_pairs = {tx_rx_pairs}, tx_users = {tx_users}, RB_needed_UL = {RB_needed_UL}, RB_needed_DL = {RB_needed_DL} and {deployment} deployment")
    
    print(f"\nRR started for {I} users , coverage = {drones_coverage} with update_loss = {packet_update_loss}, sample_loss = {packet_sample_loss}, periodicity = {periodicity}, tx_rx_pairs = {tx_rx_pairs}, tx_users = {tx_users},  RB_needed_UL = {RB_needed_UL}, RB_needed_DL = {RB_needed_DL}  and {deployment} deployment", file = open(folder_name + "/results.txt", "a"), flush = True)
    ...
    for ep in range(random_episodes): # how many times this policy will be run, similar to episode
        ...
        for x in range(MAX_STEPS):
            # print(x)
            # print("inside RR - ", x, " step started")      ## runs MAX_STEPS times       
            selected_action = find_rr_action(eval_env, eval_env.current_step)
            eval_env.prev_action = selected_action
            ...
            if verbose:
                # print(f"\n\ncurrent episode = {ep}\n")
                print(f"RR selection is {selected_action}, actual action is {action}, upload_users = {upload_users}, download_user_pairs = {download_user_pairs}, BS_age = {eval_env.UAV_age}, dest_age = {eval_env.dest_age}\n") 
    ...
    pickle.dump(attempt_sample, open(folder_name + "/" + deployment + "/" + str(I) + "U_rr_attempt_sample.pickle", "wb"))
    pickle.dump(success_sample, open(folder_name + "/" + deployment + "/" + str(I) + "U_rr_success_sample.pickle", "wb"))
    pickle.dump(attempt_update, open(folder_name + "/" + deployment + "/" + str(I) + "U_rr_attempt_update.pickle", "wb"))
    pickle.dump(success_update, open(folder_name + "/" + deployment + "/" + str(I) + "U_rr_success_update.pickle", "wb"))
    ...
    print("\nrr scheduling ", deployment, " placement, ", I, " users - MEAN of final_step_rewards = ", np.mean(final_step_rewards), " MIN and MAX of final_step_rewards = ", np.min(final_step_rewards),", ", np.max(final_step_rewards), " and avg of overall_ep_reward = ", np.mean(overall_ep_reward), ". Similarly for final_step_UAV_rewards - MEAN =",np.mean(final_step_UAV_rewards), ", MIN and MAX of final_step_UAV_rewards = ", np.min(final_step_UAV_rewards),", ", np.max(final_step_UAV_rewards)," end with final state of ", eval_env._state, " with shape ", np.shape(eval_env._state))
