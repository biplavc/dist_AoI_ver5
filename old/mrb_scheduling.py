...
def find_mrb_action(eval_env, slot): ## maximal age difference
    ...
    # mrb_age_dest = {tuple(x):(eval_env.dest_age[tuple(x)] - eval_env.UAV_age[x[0]]) for x in eval_env.tx_rx_pairs}
    sampleDict_copy = copy.deepcopy(eval_env.RB_needed_DL)
    
    mrb_user_pairs = []
    while len(mrb_user_pairs) < len(eval_env.tx_rx_pairs):
        ...
        remaining_capacity = len(eval_env.tx_rx_pairs) - len(mrb_user_pairs)
        if remaining_capacity > 0:
            mrb_user_pairs.extend(random.sample(listOfKeys, len(listOfKeys)))
        for items in listOfKeys:
            del sampleDict_copy[items]
            
    mrb_user_pairs_actual = mrb_user_pairs[:eval_env.RB_total_DL]
            
    ## random select DL pairs end
    
    download_user_pairs = mrb_user_pairs_actual
 ...
 ############################################
    ...
    actual_mrb_action = None # this has to change
    ...
    for action in range(eval_env.action_size):
        ...
        if (list(eval_action[0]))==(upload_users) and (list(eval_action[1]))==(download_user_pairs_arr):
            actual_mrb_action = action

            if verbose:
                print(f"upload_users={upload_users}, download_user_pairs={download_user_pairs_arr}, actual_mrb_action = {actual_mrb_action}")    
                
            break
        
    assert actual_mrb_action!=None
        
    return actual_mrb_action    


def mrb_scheduling(I, drones_coverage, folder_name, deployment, packet_update_loss, packet_sample_loss, periodicity, adj_matrix, tx_rx_pairs, tx_users, RB_needed_UL, RB_needed_DL):  ## maximal age difference
    
    print(f"\nMRB started for {I} users , coverage = {drones_coverage} with update_loss = {packet_update_loss}, sample_loss = {packet_sample_loss}, periodicity = {periodicity}, tx_rx_pairs = {tx_rx_pairs}, tx_users = {tx_users}, RB_needed_UL = {RB_needed_UL}, RB_needed_DL = {RB_needed_DL} and {deployment} deployment")
    
    print(f"\nMRB started for {I} users , coverage = {drones_coverage} with update_loss = {packet_update_loss}, sample_loss = {packet_sample_loss}, periodicity = {periodicity}, tx_rx_pairs = {tx_rx_pairs}, tx_users = {tx_users},  RB_needed_UL = {RB_needed_UL}, RB_needed_DL = {RB_needed_DL}  and {deployment} deployment", file = open(folder_name + "/results.txt", "a"), flush = True)
    ...
    for ep in range(random_episodes): # how many times this policy will be run, similar to episode
        ...
        if ep==0:
            print(f"\nmrb scheduling and {deployment} placement with {I} users, coverage is {eval_env.act_coverage}, RB_total_UL is {RB_total_UL}, RB_total_DL = {RB_total_DL}, action space size is {eval_env.action_size} \n\n", file = open(folder_name + "/action_space.txt", "a"), flush = True)
            
            print(f"\nmrb scheduling and {deployment} placement with {I} users, coverage is {eval_env.act_coverage}, RB_total_UL is {RB_total_UL}, RB_total_DL = {RB_total_DL}, action space size is {eval_env.action_size} ", file = open(folder_name + "/results.txt", "a"), flush = True)
        ...
        for x in range(MAX_STEPS):
            # print(x)
            # print("inside mrb - ", x, " step started")      ## runs MAX_STEPS times       
            selected_action = find_mrb_action(eval_env, eval_env.current_step)
            ...
            if verbose:
                # print(f"\n\ncurrent episode = {ep}\n")
                print(f"mrb selection is {selected_action}, actual action is {action}, upload_users = {upload_users}, download_user_pairs = {download_user_pairs}, BS_age = {eval_env.UAV_age}, dest_age = {eval_env.dest_age}\n") 
    ...
    pickle.dump(overall_ep_reward, open(folder_name + "/" + deployment + "/" + str(I) + "U_mrb_overall_ep_reward.pickle", "wb"))
    pickle.dump(final_step_UAV_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_mrb_final_step_UAV_rewards.pickle", "wb"))
    pickle.dump(final_step_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_mrb_final_step_rewards.pickle", "wb"))

    pickle.dump(age_dist_UAV, open(folder_name + "/" + deployment + "/" + str(I) + "U_mrb_age_dist_UAV.pickle", "wb"))
    pickle.dump(age_dist_dest, open(folder_name + "/" + deployment + "/" + str(I) + "U_mrb_age_dist_dest.pickle", "wb"))
    
    pickle.dump(eval_env.sample_time, open(folder_name + "/" + deployment + "/" + str(I) + "U_mrb_sample_time.pickle", "wb"))
    
    pickle.dump(attempt_sample, open(folder_name + "/" + deployment + "/" + str(I) + "U_mrb_attempt_sample.pickle", "wb"))
    pickle.dump(success_sample, open(folder_name + "/" + deployment + "/" + str(I) + "U_mrb_success_sample.pickle", "wb"))
    pickle.dump(attempt_update, open(folder_name + "/" + deployment + "/" + str(I) + "U_mrb_attempt_update.pickle", "wb"))
    pickle.dump(success_update, open(folder_name + "/" + deployment + "/" + str(I) + "U_mrb_success_update.pickle", "wb"))
    
    pickle.dump(unutilized_RB_DL, open(folder_name + "/" + deployment + "/" + str(I) + "U_mrb_unutilized_RB_DL.pickle", "wb"))
    pickle.dump(unutilized_RB_UL, open(folder_name + "/" + deployment + "/" + str(I) + "U_mrb_unutilized_RB_UL.pickle", "wb"))
    
    
    print("\nmrb scheduling ", deployment, " placement, ", I, " users - MEAN of final_step_rewards = ", np.mean(final_step_rewards), " MIN and MAX of final_step_rewards = ", np.min(final_step_rewards),", ", np.max(final_step_rewards), " and avg of overall_ep_reward = ", np.mean(overall_ep_reward), ". Similarly for final_step_UAV_rewards - MEAN =",np.mean(final_step_UAV_rewards), ", MIN and MAX of final_step_UAV_rewards = ", np.min(final_step_UAV_rewards),", ", np.max(final_step_UAV_rewards)," end with final state of ", eval_env._state, " with shape ", np.shape(eval_env._state))
